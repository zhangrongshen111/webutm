...
$(function(){
    $(".btn").click(function(){
        ...
        if(!isRegisterUserName(userName)){
            ...
            $(".input_div7 span").html('<img src="images/register/text_error.png"><font color=red>输入图片验证码</font>').fadeIn();
            $("#varcode").focus();
            $(".btn").val('注册').removeAttr('disabled');
            return false;
        } else if(agreenMent != '1'){
            //$(".agreenment-tips").html('请先同意用户条款!').fadeIn();
            $(".input_div8 span").html('<img src="images/register/text_error.png"><font color=red>请先同意用户条款!</font>');
            $(".btn").val('注册').removeAttr('disabled');
            return false;
        }else{
